...
export async function GET() {
  ...
  const sperre = await pruefeFunktion(supabase, kontoId, 'lernschleife')
  ...
  const { beobachtungen, angebote } = await sammleBeobachtungen(supabase, kontoId)
...
export async function POST() {
  ...
  const s = await speichereKalibrierung(supabase, kontoId, {
