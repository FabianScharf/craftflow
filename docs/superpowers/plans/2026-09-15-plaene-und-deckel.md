...
- Modify: `/api/usage` POST — **Entschieden (Fabian, 16.09.): ein Angebot zählt bei der Analyse**, nicht beim PDF — die Kalkulation ist dann exportierbar und steht in der Projektliste. Umsetzung: `analyze/route.ts` prüft VOR dem KI-Aufruf den Deckel (`pruefeDeckel(plan, 'angebote', count)`), zählt NACH erfolgreichem KI-Aufruf hoch (`plan_usage.angebote_count + 1`); der Aufruf von `incrementUsage()` beim PDF (`page.tsx:4470`) entfällt, die Anzeige „Noch N von M Angeboten“ bleibt und liest `/api/usage` GET.
